...
def stddpg(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
           ...
           max_ep_len=1000, logger_kwargs=dict(), save_freq=1, qleader=True, decay=True, reg=0, reg_decay=1e5,
           rollout_iters=1):
    """
    ...
    def compute_loss_q(data, targ_pi=True):
        o, a, r, o2, d = data['obs'].to(device), data['act'].to(device), data['rew'].to(device), data['obs2'].to(
            device), data['done'].to(device)
