...
def ddpg(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    def calculate_s_actor_loss(data):
        ...
        dq_dthetas = autograd.grad(outputs=q,
                                   ...
                                   create_graph=True)
        # vectorize
        dq_dw_vec = torch.zeros(size=[1]).to(device)
        dq_dtheta_vec = torch.zeros(size=[1]).to(device)
        for dq_dw in dq_dws:
            dq_dw = torch.flatten(dq_dw)
        dq_dw_vec = torch.cat((dq_dw_vec, dq_dw), dim=0)
        dq_dw_vec = dq_dw_vec[1:].unsqueeze(0)  # delete the first element
        for dq_dtheta in dq_dthetas:
            dq_dtheta = torch.flatten(dq_dtheta)
        dq_dtheta_vec = torch.cat((dq_dtheta_vec, dq_dtheta), dim=0)
        dq_dtheta_vec = dq_dtheta_vec[1:].unsqueeze(0)  # delete the first
        element
        k_1 = 2 * torch.transpose(dq_dw_vec, dim0=0, dim1=1) @ dq_dtheta_vec  #Theta * w
        # deal with the diagonal block
        k_2_dia = 2 * dq_dw_vec * dq_dw_vec  # w*w
        rec_k_2_dia = torch.reciprocal(k_2_dia + 1e-5)
        # calculate the gradient for actor
        dq_dtheta_stack = torch.transpose(dq_dtheta_vec,
                                          dim0=0, dim1=1) + torch.transpose(k_1, dim0=0, dim1=1) * \
                        rec_k_2_dia @ torch.transpose(dq_dw_vec, dim0=0, dim1=1)
        dq_dtheta_stack = dq_dtheta_stack[:,0]
        for i, para in enumerate(ac.pi.parameters()):
            para.grad = dq_dtheta_stack[param_index[i]:param_index[i + 1]].reshape(para.shape)
